...
I decicate roughly 50% of my time working around infrastructure topics (contributing to our ansible codebase, incident diagnosis and a few operationnal tasks), 
spending time with SRE teams to architecture, reproduce bugs and so on.
\\
Challenges are quite interestings: we have to ensure and maintain service availability while delivering (sometimes not so stable) features - each compromise has to be evaluated, a mistake or some misunderstood quickly happen within a large Python codebase and environment.
